...
REM Continue with the script if dependencies are installed
...
docker network inspect "student-placement-platform" > nul 2>&1
if %errorlevel% neq 0 (
...
:wait_for_mysql
docker exec mysql-database mysql -uroot -p"%DB_ROOT_PASSWORD%" -e "status" > nul 2>&1
if %errorlevel% neq 0 (
